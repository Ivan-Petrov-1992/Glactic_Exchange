...
export default function PoolDept({ t, data }) {
  ...
  const maxHandler = (e) => {
    e.preventDefault();
    if (data) {
      inputRef.current.value = Number(data.data.gUSD_balance);
    }
  };
    ...
    >
      ...
      >
        <Flex w="100%" h="50%" align="center">
          ...
          <Spacer />
          {data ? (
            <Text color="#666666" m="0" fontSize="14px" pr="2rem">
              {t().stable.balance}: {Number(data.data.gUSD_balance).toFixed(2)}
              gUSD
            </Text>
          ) : (
            <Text pr="2rem">Loading...</Text>
          )}
        </Flex>
        <Flex w="100%" h="50%" align="center">
          <Input
